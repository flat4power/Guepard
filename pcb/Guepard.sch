EESchema Schematic File Version 2  date 19/06/2013 13:36:50
LIBS:Guepard
LIBS:Guepard-Communication
LIBS:opto
LIBS:power
LIBS:conn
LIBS:transistors
LIBS:polyswitch_smd
LIBS:device
LIBS:texas-msp
LIBS:Guepard-cache
EELAYER 27 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 8
Title "Guepard PCB for FreeEMS"
Date "19 jun 2013"
Rev "0.1-alpha"
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text GLabel 6250 4400 2    40   Input ~ 0
Injector-GND
Text HLabel 9300 6550 2    40   Input ~ 0
RPM0-IN-GND
Text GLabel 9300 6550 0    40   Input ~ 0
RPM-Crank-IN-GND
Text GLabel 6250 1250 2    40   Input ~ 0
RPM-Crank-IN-GND
Text GLabel 6800 4550 2    40   Output ~ 0
Output-Ground
Text GLabel 9050 3400 0    40   Input ~ 0
Output-Ground
Text GLabel 6250 6350 2    40   Input ~ 0
Ignitor4
Text GLabel 6250 5900 2    40   Input ~ 0
Ignitor3
Text GLabel 6250 6050 2    40   Input ~ 0
Ignitor2
Text GLabel 6250 6200 2    40   Input ~ 0
Ignitor1
Text GLabel 6250 5150 2    40   Input ~ 0
Injector-GND
Text GLabel 9050 1850 0    40   Input ~ 0
Injector-GND
Text GLabel 6250 4850 2    40   Input ~ 0
Injector2/4
Text GLabel 6250 5000 2    40   Input ~ 0
Injector1/3
Text GLabel 10600 800  2    40   Input ~ 0
Injector2/4
Text GLabel 10600 700  2    40   Input ~ 0
Injector1/3
Text GLabel 7050 2600 2    40   Input ~ 0
O2-GND
Text GLabel 6250 2750 2    40   Input ~ 0
TPS-GND
Text GLabel 6250 3200 2    40   Input ~ 0
TPS-5V
Text GLabel 9200 4200 0    40   Input ~ 0
Spare-DIN-2
Text GLabel 9200 4100 0    40   Input ~ 0
Spare-DIN-1
Text GLabel 9200 3750 0    40   Input ~ 0
+12V-ACC
Text HLabel 9200 3750 2    40   Output ~ 0
BRV-12v
Text GLabel 6250 3500 2    40   Input ~ 0
Spare-LSD-1
Text GLabel 6250 3950 2    40   Input ~ 0
Spare-LSD-2
Text GLabel 10650 3100 2    40   Output ~ 0
Spare-LSD-1
Text GLabel 10650 3000 2    40   Output ~ 0
Spare-LSD-2
Text GLabel 10650 2600 2    40   Input ~ 0
Ignitor4
Text GLabel 10650 2500 2    40   Input ~ 0
Ignitor3
Text GLabel 10650 2400 2    40   Input ~ 0
Ignitor2
Text GLabel 10650 2300 2    40   Input ~ 0
Ignitor1
Text GLabel 8550 2200 0    40   Input ~ 0
+12V-SWITCH
Text GLabel 6800 5600 2    40   Input ~ 0
GROUND
Text GLabel 6250 5450 2    40   Input ~ 0
+12V-SWITCH
Text GLabel 9300 4900 0    40   Input ~ 0
+12V-ACC
Text GLabel 10650 2900 2    40   Output ~ 0
FP-Relay
Text GLabel 6250 3800 2    40   Input ~ 0
FP-Relay
Text GLabel 1650 5200 0    40   Input ~ 0
+12V-SWITCH
Text GLabel 6250 5300 2    40   Input ~ 0
+12V-ACC
Text GLabel 6250 2450 2    40   Input ~ 0
SPR-Sensor
Text GLabel 6250 1550 2    40   Input ~ 0
RPM-Crank-IN-
Text GLabel 6250 1400 2    40   Input ~ 0
RPM-Crank-IN+
Text GLabel 6250 2600 2    40   Input ~ 0
O2-Sensor
Text GLabel 6250 2900 2    40   Input ~ 0
TPS-Sensor
Text GLabel 6250 3050 2    40   Input ~ 0
Coolant-Sensor
Text GLabel 6250 3350 2    40   Input ~ 0
IAT-Sensor
Text GLabel 9300 6450 0    40   Input ~ 0
RPM-Crank-IN-
Text GLabel 9300 6350 0    40   Input ~ 0
RPM-Crank-IN+
Text GLabel 9300 6000 0    40   Input ~ 0
O2-GND
Text GLabel 9300 5900 0    40   Input ~ 0
O2-Sensor
Text GLabel 9300 5650 0    40   Input ~ 0
TPS-GND
Text GLabel 9300 5550 0    40   Input ~ 0
TPS-Sensor
Text GLabel 9300 5450 0    40   Input ~ 0
TPS-5v
Text GLabel 9300 5350 0    40   Input ~ 0
Coolant-Sensor
Text GLabel 9300 5250 0    40   Input ~ 0
SPR-Sensor
Text GLabel 9300 5150 0    40   Input ~ 0
IAT-Sensor
Text GLabel 3200 1650 2    40   Output ~ 0
Fuel-Pump-Drive
Text GLabel 9050 2700 0    40   Input ~ 0
Ignitor4-IN
Text GLabel 9050 2600 0    40   Input ~ 0
Ignitor3-IN
Text GLabel 9050 2500 0    40   Input ~ 0
Ignitor2-IN
Text GLabel 9050 2400 0    40   Input ~ 0
Ignitor1-IN
Text GLabel 9200 3950 0    40   Input ~ 0
5vdc
Text GLabel 3200 950  2    40   Output ~ 0
Ignitor1-IN
Text GLabel 3200 1050 2    40   Output ~ 0
Ignitor2-IN
Text GLabel 10650 5050 2    40   Output ~ 0
AAP
Text GLabel 1500 2400 0    40   Input ~ 0
AAP
Text GLabel 9050 2300 0    40   Input ~ 0
5vdc
Text GLabel 1650 5400 0    40   Input ~ 0
Ground
Text GLabel 1500 2600 0    40   Input ~ 0
Ground
Text GLabel 10600 1550 2    40   Input ~ 0
Ground
Text Notes 1150 5150 0    40   ~ 0
12v-Constant
Text GLabel 3200 1450 2    40   Output ~ 0
Injector2/4-IN
Text GLabel 3200 1350 2    40   Output ~ 0
Injector1/3-IN
Text GLabel 3200 1250 2    40   Output ~ 0
Ignitor4-IN
Text GLabel 3200 1150 2    40   Output ~ 0
Ignitor3-IN
Text GLabel 9050 1000 0    40   Input ~ 0
Injector2/4-IN
Text GLabel 9050 900  0    40   Input ~ 0
Injector1/3-IN
$Sheet
S 9300 4850 1350 1900
U 4F0BB08D
F0 "Inputs" 40
F1 "inputs.sch" 40
F2 "O2-Sensor-GND" I L 9300 6000 40 
F3 "TPS-Sensor-GND" I L 9300 5650 40 
F4 "Coolant-Sensor" I L 9300 5350 40 
F5 "Coolant-CPU-Signal" O R 10650 5450 40 
F6 "O2-Sensor" I L 9300 5900 40 
F7 "O2-CPU-Signal" O R 10650 5550 40 
F8 "IAT-CPU-Signal" O R 10650 5250 40 
F9 "IAT-Sensor" I L 9300 5150 40 
F10 "TPS-CPU-Signal" O R 10650 5350 40 
F11 "TPS-Sensor" I L 9300 5550 40 
F12 "SPR-Sensor" I L 9300 5250 40 
F13 "SPR-CPU-Signal" O R 10650 5650 40 
F14 "BRV" O R 10650 4900 40 
F15 "MAP-CPU-Signal" O R 10650 5150 40 
F16 "BRV-12v" I L 9300 4900 40 
F17 "TPS-5vdc" O L 9300 5450 40 
F18 "5vdc" I L 9300 5000 40 
F19 "RPM0-IN-" B L 9300 6450 40 
F20 "RPM0-IN+" B L 9300 6350 40 
F21 "RPM0-OUT" O R 10650 6550 40 
F22 "AAP-CPU-Signal" O R 10650 5050 40 
$EndSheet
Text GLabel 9300 5000 0    40   Input ~ 0
5vdc
Text GLabel 1500 2000 0    40   Input ~ 0
BRV
Text GLabel 9050 2800 0    40   Input ~ 0
Fuel-Pump-Drive
$Sheet
S 9050 2150 1600 1350
U 4F0D82A1
F0 "Outputs" 40
F1 "Outputs.sch" 40
F2 "FP-Relay" O R 10650 2900 40 
F3 "Spare-LSD-2" O R 10650 3000 40 
F4 "PA7" I L 9050 2800 40 
F5 "Spare-LSD-1" O R 10650 3100 40 
F6 "12vdc-switched-outputs" I L 9050 2200 40 
F7 "5vdc" I L 9050 2300 40 
F8 "Ignitor1" O R 10650 2300 40 
F9 "Ignitor2" O R 10650 2400 40 
F10 "Ignitor3" O R 10650 2500 40 
F11 "Ignitor4" O R 10650 2600 40 
F12 "PT2" I L 9050 2400 40 
F13 "PT3" I L 9050 2500 40 
F14 "PT4" I L 9050 2600 40 
F15 "PT5" I L 9050 2700 40 
F16 "PP3" I L 9050 3000 40 
F17 "PP5" I L 9050 3100 40 
F18 "Output-Ground" I L 9050 3400 40 
$EndSheet
$Sheet
S 9050 650  1550 1350
U 4F0D0381
F0 "Injectors" 40
F1 "Injectors.sch" 40
F2 "PT6" I L 9050 900 40 
F3 "Injector2/4_Output" O R 10600 800 40 
F4 "Injector1/3_Output" O R 10600 700 40 
F5 "PT7" I L 9050 1000 40 
F6 "12v-sw2" I L 9050 1500 40 
F7 "PCB-GND" I R 10600 1550 40 
F8 "Injector-Ground" I L 9050 1850 40 
$EndSheet
$Sheet
S 1650 5100 1050 400 
U 4F0A4778
F0 "Power Supplies" 40
F1 "PowerSupplies.sch" 40
F2 "12v-switch" I L 1650 5200 40 
F3 "5vdc" O R 2700 5200 40 
F4 "PWR-GND" I L 1650 5400 40 
$EndSheet
Text GLabel 10650 6550 2    40   Output ~ 0
RPM0
Text GLabel 1500 2200 0    40   Input ~ 0
O2
Text GLabel 10650 5550 2    40   Output ~ 0
O2
Text GLabel 1500 2100 0    40   Input ~ 0
TPS
Text GLabel 10650 5350 2    40   Output ~ 0
TPS
Text GLabel 1500 1800 0    40   Input ~ 0
COOLANT
Text GLabel 10650 5450 2    40   Output ~ 0
COOLANT
Text GLabel 1500 2300 0    40   Input ~ 0
SPR
Text GLabel 10650 5650 2    40   Output ~ 0
SPR
Text GLabel 1500 1700 0    40   Input ~ 0
IAT
Text GLabel 10650 5250 2    40   Output ~ 0
IAT
Text GLabel 10650 5150 2    40   Output ~ 0
MAP
Text GLabel 10650 4900 2    40   Output ~ 0
BRV
Text GLabel 1500 1450 0    40   Input ~ 0
RPM0
Text GLabel 1500 1900 0    40   Input ~ 0
MAP
Text GLabel 1500 950  0    40   Input ~ 0
5vdc
Text GLabel 2700 5200 2    40   Output ~ 0
5vdc
Text GLabel 1550 3650 0    40   Input ~ 0
5vdc
Text GLabel 1550 4500 0    40   Input ~ 0
Ground
Text GLabel 3200 2000 2    40   Output ~ 0
CPU-TXCAN0
Text GLabel 3200 2100 2    40   Input ~ 0
CPU-RXCAN0
Text GLabel 3200 2200 2    40   Output ~ 0
CPU-TXD1
Text GLabel 3200 2300 2    40   Input ~ 0
CPU-RXD1
Text GLabel 3200 2400 2    40   Output ~ 0
CPU-TXD0
Text GLabel 3200 2500 2    40   Input ~ 0
CPU-RXD0
Text GLabel 1550 3800 0    40   Input ~ 0
CPU-TXCAN0
Text GLabel 1550 3900 0    40   Output ~ 0
CPU-RXCAN0
Text GLabel 1550 4000 0    40   Input ~ 0
CPU-TXD1
Text GLabel 1550 4100 0    40   Output ~ 0
CPU-RXD1
Text GLabel 1550 4200 0    40   Input ~ 0
CPU-TXD0
Text GLabel 1550 4300 0    40   Output ~ 0
CPU-RXD0
$Sheet
S 1500 900  1700 2200
U 4F0A2F06
F0 "CPU" 40
F1 "cpu.sch" 40
F2 "SPR-Signal" I L 1500 2300 40 
F3 "BRV-Signal" I L 1500 2000 40 
F4 "MAP-Signal" I L 1500 1900 40 
F5 "O2-Signal" I L 1500 2200 40 
F6 "TPS-Signal" I L 1500 2100 40 
F7 "Coolant-Signal" I L 1500 1800 40 
F8 "IAT-Signal" I L 1500 1700 40 
F9 "AAP-Signal" I L 1500 2400 40 
F10 "5vdc" I L 1500 950 40 
F11 "PT4" O R 3200 1150 40 
F12 "PT3" O R 3200 1050 40 
F13 "PT2" O R 3200 950 40 
F14 "PT5" O R 3200 1250 40 
F15 "PCB-GND" I L 1500 2600 40 
F16 "RPM0" I L 1500 1450 40 
F17 "PT6" O R 3200 1350 40 
F18 "PT7" O R 3200 1450 40 
F19 "TXCAN0" O R 3200 2000 40 
F20 "RXCAN0" I R 3200 2100 40 
F21 "TXD1" O R 3200 2200 40 
F22 "RXD1" I R 3200 2300 40 
F23 "TXD0" O R 3200 2400 40 
F24 "RXD0" I R 3200 2500 40 
F25 "PP3" O R 3200 1750 40 
F26 "PA7" O R 3200 1650 40 
F27 "PP5" O R 3200 1850 40 
$EndSheet
$Comp
L TE-776163 J5
U 1 1 51B65F50
P 6250 3700
F 0 "J5" H 6100 6700 60  0000 C CNN
F 1 "TE-776163" H 6100 6500 60  0000 C CNN
F 2 "~" H 6250 3700 60  0000 C CNN
F 3 "~" H 6250 3700 60  0000 C CNN
	1    6250 3700
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR01
U 1 1 51B6652C
P 6700 5800
F 0 "#PWR01" H 6700 5800 30  0001 C CNN
F 1 "GND" H 6700 5730 30  0001 C CNN
F 2 "" H 6700 5800 60  0000 C CNN
F 3 "" H 6700 5800 60  0000 C CNN
	1    6700 5800
	1    0    0    -1  
$EndComp
Text GLabel 6250 2000 2    40   Input ~ 0
Spare-DIN-1
Text GLabel 6250 1700 2    40   Input ~ 0
Spare-DIN-2
Wire Wire Line
	9050 1500 8550 1500
Wire Wire Line
	8550 1500 8550 2200
Wire Wire Line
	8550 2200 9050 2200
Connection ~ 8550 2200
Connection ~ 9050 2200
Connection ~ 9050 1500
Wire Wire Line
	6250 5600 6800 5600
Connection ~ 6700 5600
Wire Wire Line
	6950 1850 6950 2750
$Comp
L GND #PWR02
U 1 1 51B6B98E
P 6950 2750
F 0 "#PWR02" H 6950 2750 30  0001 C CNN
F 1 "GND" H 6950 2680 30  0001 C CNN
F 2 "" H 6950 2750 60  0000 C CNN
F 3 "" H 6950 2750 60  0000 C CNN
	1    6950 2750
	1    0    0    -1  
$EndComp
$Sheet
S 1550 3500 1600 1100
U 51A60BA0
F0 "Communication" 50
F1 "Guepard-Communication.sch" 50
F2 "TX_CAN" I L 1550 3800 40 
F3 "RX_can" O L 1550 3900 40 
F4 "RX_BT" I L 1550 4000 40 
F5 "TX_BT" O L 1550 4100 40 
F6 "RX_USB" I L 1550 4200 40 
F7 "TX_USB" O L 1550 4300 40 
F8 "PCB-GND" I L 1550 4500 40 
F9 "5vdc" I L 1550 3650 40 
$EndSheet
Text GLabel 3200 1750 2    40   Output ~ 0
PP3
Text GLabel 3200 1850 2    40   Output ~ 0
PP5
Text GLabel 9050 3000 0    40   Input ~ 0
PP3
Text GLabel 9050 3100 0    40   Input ~ 0
PP5
Wire Wire Line
	6700 5600 6700 5800
Wire Wire Line
	6250 5750 6700 5750
Connection ~ 6700 5750
Wire Wire Line
	6800 4550 6250 4550
Wire Wire Line
	6750 4700 6250 4700
Wire Wire Line
	6750 4250 6750 4700
Connection ~ 6750 4550
Wire Wire Line
	6250 4250 6750 4250
Wire Wire Line
	6250 1850 6950 1850
Wire Wire Line
	7050 2600 6950 2600
Connection ~ 6950 2600
Wire Wire Line
	6250 2300 6950 2300
Connection ~ 6950 2300
$Sheet
S 9200 3700 800  550 
U 51767E16
F0 "Digital-Inputs" 60
F1 "Digital-Inputs.sch" 60
F2 "Spare-DIN-1" I L 9200 4100 40 
F3 "Spare-DIN-2" I L 9200 4200 40 
F4 "5vdc" I L 9200 3950 40 
$EndSheet
$EndSCHEMATC
